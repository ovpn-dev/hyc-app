...
export const useLikedQuotes = () => {
  ...
  useEffect(() => {
    const loadLikedQuotes = async () => {
      try {
        const storedQuotes = await StorageHelpers.getItem(
          STORAGE_KEYS.LIKED_QUOTES,
          []
        );
        setLikedQuotes(storedQuotes);
      } catch (error) {
  ...
  const saveLikedQuotes = useCallback(async (quotes: LikedQuote[]) => {
    try {
      await StorageHelpers.setItem(STORAGE_KEYS.LIKED_QUOTES, quotes);
    } catch (error) {
  ...
  const addLikedQuote = useCallback(
    (quoteText: string, author: string) => {
      setLikedQuotes((prev) => {
  ...
  const removeLikedQuote = useCallback(
    (quoteText: string) => {
      setLikedQuotes((prev) => {
